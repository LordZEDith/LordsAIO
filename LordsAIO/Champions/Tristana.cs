    ...
    internal class Tristana : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            Program.Menu.SubMenu("Passive")
                .AddItem(new MenuItem("useR_Antigapclose", "Use R AntigapcloseS").SetValue(true));
        ...
        private bool ComboKs()
        {
            ...
            foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsValidTarget() && !hero.IsAlly))
            {
                ...
                if (W.IsReady() && Program.Menu.Item("use_ComboKS_W").GetValue<bool>())
                {
                    ...
                    if (health <= igniteDamage &&
                        targetdis < W.Range + Orbwalking.GetRealAutoAttackRange(ObjectManager.Player) - 200)
                    {
                        Program.Orbwalker.ForceTarget(enemy);
                        W.Cast(GetJumpposition(enemy));
                        return true;
                    }
        ...
        private void CastQ_farm()
        {
            ...
            if (
                allMinions.Where(minion => minion != null)
                    .Any(
                        minion =>
                            minion.IsValidTarget(Orbwalking.GetRealAutoAttackRange(ObjectManager.Player)) && Q.IsReady()))
            {
                Q.Cast();
            }
        }
        ...
        {
            if (!R.IsReady() || !target.IsValidTarget(R.Range) || target.IsAlly ||
        ...
        private void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                Drawing.DrawCircle(GetJumpposition(tar), 150, Color.Blue);
            }
            ...
            if (Program.Menu.Item("Draw_E").GetValue<bool>())
            {
                ...
                {
                    Drawing.DrawCircle(
                        ObjectManager.Player.Position, E.Range - 1, E.IsReady() ? Color.Green : Color.Red);
                }
            }
            ...
            if (Program.Menu.Item("Draw_R").GetValue<bool>())
            {
                ...
                {
                    Drawing.DrawCircle(
                        ObjectManager.Player.Position, R.Range - 2, R.IsReady() ? Color.Green : Color.Red);
                }
            }
        }
    }
