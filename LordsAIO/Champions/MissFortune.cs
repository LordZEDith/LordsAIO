    ...
    internal class MissFortune : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useE_TeamFight_bind", "Use E if stunned").SetValue(true));
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useE_TeamFight_willhit", "Use E if hit").SetValue(new Slider(1, 5, 0)));
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useR_TeamFight_willhit", "Use R if Hit").SetValue(new Slider(3, 5, 0)));
            ...
            Program.Menu.SubMenu("Harass")
                .AddItem(new MenuItem("useE_Harass_willhit", "Use E if hit").SetValue(new Slider(2, 5, 0)));
            ...
            if (
                !Program.Helper.EnemyTeam.Where(hero => hero.IsValidTarget(R.Range))
                    .Any(
                        enemy =>
                            R.CastIfWillHit(
                                enemy, Program.Menu.Item("useR_TeamFight_willhit").GetValue<Slider>().Value - 1
                              )))
            {
                return;
            }
        ...
        private void CastEEnemyAmount()
        {
            ...
            {
                E.CastIfWillHit(
                    enemy, Program.Menu.Item("useE_TeamFight_willhit").GetValue<Slider>().Value - 1);
                return;
            }
        }
        ...
        private void CastW()
        {
            ...
            var target = TargetSelector.GetTarget(
                Orbwalking.GetRealAutoAttackRange(ObjectManager.Player), TargetSelector.DamageType.Physical);
            if (target != null)
            {
                W.Cast();
            }
            ...
            var allMinion = MinionManager.GetMinions(
                ObjectManager.Player.Position, Orbwalking.GetRealAutoAttackRange(ObjectManager.Player), MinionTypes.All,
                MinionTeam.NotAlly);
        ...
        private void CastEMinion()
        {
            ...
            var minions = MinionManager.GetMinions(
                ObjectManager.Player.Position, E.Range + (E.Width + 200 / 2), MinionTypes.All, MinionTeam.NotAlly);
            if (minions.Count == 0)
            {
                return;
            }
            var castPostion =
                MinionManager.GetBestCircularFarmLocation(
                    minions.Select(minion => minion.ServerPosition.To2D()).ToList(), E.Width + 200, E.Range);
            E.Cast(castPostion.Position);
        }
    }
}