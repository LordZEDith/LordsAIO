    ...
    internal class Zyra : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useR_TeamFight_willhit", "Use R if hit").SetValue(new Slider(2, 5, 0)));
            ...
            Program.Menu.SubMenu("Passive")
                .AddItem(new MenuItem("useW_Passive", "Plant on Spelllocations").SetValue(true));
        ...
        private void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                Drawing.DrawCircle(
                    ObjectManager.Player.Position, Passive.Range, Passive.IsReady() ? Color.Green : Color.Red);
                return;
            }
        ...
        private void CastEMinion()
        {
            ...
            var minions = MinionManager.GetMinions(
                ObjectManager.Player.Position, E.Range, MinionTypes.All, MinionTeam.NotAlly);
            if (minions.Count == 0)
            {
                return;
            }
            var castPostion =
                MinionManager.GetBestLineFarmLocation(
                    minions.Select(minion => minion.ServerPosition.To2D()).ToList(), E.Width, E.Range);
        ...
        private void CastQMinion()
        {
            ...
            var minions = MinionManager.GetMinions(
                ObjectManager.Player.Position, Q.Range + (Q.Width / 2), MinionTypes.All, MinionTeam.NotAlly);
            if (minions.Count == 0)
            {
                return;
            }
            var castPostion =
                MinionManager.GetBestCircularFarmLocation(
                    minions.Select(minion => minion.ServerPosition.To2D()).ToList(), Q.Width, Q.Range);
