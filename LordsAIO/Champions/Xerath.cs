...
namespace LordsAIO.Champions
{
    internal class Xerath : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            Program.Menu.SubMenu("Passive")
                .AddItem(
                    new MenuItem("useR_KS", "Use R for KS").SetValue(
                        (new KeyBind("T".ToCharArray()[0], KeyBindType.Press))));
            Program.Menu.SubMenu("Passive")
                .AddItem(new MenuItem("useR_safe", "Saferange").SetValue(new Slider(700, 2000, 0)));
            Program.Menu.SubMenu("Passive")
                .AddItem(new MenuItem("useR_Killabletext", "Write if is killable").SetValue(true));
            Program.Menu.SubMenu("Passive")
                .AddItem(new MenuItem("useR_Killableping", "Ping if is killable").SetValue(true));
            ...
            switch (Program.Orbwalker.ActiveMode)
            {
                ...
                case Orbwalking.OrbwalkingMode.LaneClear:
                    if (Program.Menu.Item("useQ_LaneClear").GetValue<bool>() &&
                        (ManaManagerAllowCast(Q) || Q.IsCharging))
                    {
                        QFarm();
                    }
        ...
        private void R_Check()
        {
            ...
            if (Utility.CountEnemiesInRange(Program.Menu.Item("useR_safe").GetValue<Slider>().Value) >= 1 &&
                !IsShooting())
            {
                return;
            }
            ...
            if (lowesttarget[0] != null &&
                lowesttarget[0].Health < (ObjectManager.Player.GetSpellDamage(lowesttarget[0], SpellSlot.R) * 0.9) &&
                Environment.TickCount - UltTick >= 700)
            {
                R.Cast(lowesttarget[0]);
        ...
        public void QFarm()
        {
            ...
            var minions = MinionManager.GetMinions(
                ObjectManager.Player.ServerPosition, Q.ChargedMaxRange, MinionTypes.All, MinionTeam.NotAlly);
            if (minions.Count <= 0)
            {
                return;
            }
            if (Q.IsCharging)
            {
                ...
                if (minions.Count == minions.Count(m => ObjectManager.Player.Distance(m) < Q.Range) &&
                    locQ.MinionsHit > 0 && locQ.Position.IsValid())
                {
        ...
        private void Interrupter_OnPosibleToInterrupt(Obj_AI_Hero target, Interrupter2.InterruptableTargetEventArgs args)
        {
            ...
            if (ObjectManager.Player.Distance(target) < E.Range && E.IsReady() && target.IsEnemy)
            {
        ...
        private void Drawing_OnDraw(EventArgs args)
        {
                        ...
                        ObjectManager.Player.GetSpellDamage(x.Player, SpellSlot.R) * 0.9 >= x.Player.Health))
            {
                ...
                if (!(ObjectManager.Player.Distance(target.Player) > 1800) || (!target.Player.IsVisible))
                {
                    continue;
                }
            ...
            if (victims != "" && Program.Menu.Item("useR_Killabletext").GetValue<bool>())
            {
                ...
                {
                    Drawing.DrawLine(
                        Drawing.Width * 0.44f - 50, Drawing.Height * 0.7f - 2, Drawing.Width * 0.44f + 350,
                        Drawing.Height * 0.7f - 2, 25, Color.Black);
                    Drawing.DrawText(
                        Drawing.Width * 0.44f, Drawing.Height * 0.7f, Color.GreenYellow,
                        "[Press " + Convert.ToChar(Program.Menu.Item("useR_KS").GetValue<KeyBind>().Key) +
                        "] Ult can kill: " + victims);
                }
            }
            if (victims == "")
            {
                if (Program.Menu.Item("useR_warning").GetValue<bool>() &&
                    Program.Menu.Item("useR_KS").GetValue<KeyBind>().Active)
                {
                    Drawing.DrawLine(
                        Drawing.Width * 0.44f - 50, Drawing.Height * 0.7f - 2, Drawing.Width * 0.44f + 350,
                        Drawing.Height * 0.7f - 2, 25, Color.Black);
                    Drawing.DrawText(
                        Drawing.Width * 0.44f, Drawing.Height * 0.7f, Color.OrangeRed,
                        "Warning: [Press " + Convert.ToChar(Program.Menu.Item("useR_KS").GetValue<KeyBind>().Key) +
                        "] to disable KS ");
                }
            }
        }
