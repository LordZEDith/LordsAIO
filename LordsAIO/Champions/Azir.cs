...
namespace LordsAIO.Champions
{
    internal class Azir : Champion
    {
            ...
            Program.Menu.SubMenu("Harass")
                .AddItem(
                    new MenuItem("useQ_Harass", "Use Q").SetValue(
                        new StringList(new[] { "Not", "at 1 Soldier", "at 2 Soldiers only" }, 1)));
        ...
        private void CastR()
        {
            ...
            foreach (
                var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsEnemy && hero.IsValidTarget(100)))
            {
                        ...
                        .Where(
                            unit =>
                                ...
                                unit.ServerPosition.To2D()
                                    .Distance(ObjectManager.Player.Position.To2D(), obj.Position.To2D(), true) < 80))
                {
        ...
        private void CastQCombo()
        {
                ...
                )
            {
                var target =
                    ObjectManager.Get<Obj_AI_Hero>()
                        .FirstOrDefault(enemy => enemy.IsEnemy && enemy.Distance(obj) < Q.Range);
        ...
        public Vector3 GetThrowPosition(Obj_AI_Hero target)
        {
            ...
            {
                if (!friend.IsMe && friend.Health / friend.MaxHealth * 100 > 15 && friend.IsAlly &&
                    friend.Distance(target) <= R.Range + friend.AttackRange - 25 + 400)
                {
        ...
        private void Drawing_OnDraw2(EventArgs args)
        {
            ...
            if (Program.Menu.Item("Draw_insec").GetValue<bool>())
            {
                    ...
                    ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsEnemy && hero.IsValidTarget(1300)))
                {
                    if (GetThrowPosition(enemy) != default(Vector3))
                    {
                        Drawing.DrawCircle(GetThrowPosition(enemy), 50, Color.Yellow);
                        Drawing.DrawCircle(GetThrowPosition(enemy), 75, Color.Yellow);
                        Drawing.DrawLine(
                            Drawing.WorldToScreen(GetThrowPosition(enemy)), Drawing.WorldToScreen(enemy.Position), 2,
                            Color.Yellow);
                    }
                }
            }
        ...
        private void GetSoldierCount()
        {
            SoldierCount =
                ObjectManager.Get<Obj_AI_Base>()
                    .Count(
                        obj =>
                            obj.Name == "AzirSoldier" && obj.IsAlly && (int) obj.BoundingRadius < 66 &&
                            obj.AttackSpeedMod > 1);
        }
        ...
        internal static class Orbwalking
        {
                ...
                return
                    ObjectManager.Get<Obj_AI_Minion>()
                        ...
                        .SelectMany(
                            soldier =>
                                ObjectManager.Get<Obj_AI_Hero>()
                                    .Where(hero => hero.IsValidTarget() && hero.Distance(soldier) < 350))
                        .FirstOrDefault();
            }
                ...
                return target != null &&
                       ObjectManager.Get<Obj_AI_Minion>()
                           .Any(
                               obj =>
                                   obj.Name == "AzirSoldier" && obj.IsAlly && obj.BoundingRadius < 66 &&
                                   obj.AttackSpeedMod > 1 && obj.Distance(target) < 350);
            }
            ...
            public static bool CanAttack()
            {
                ...
                {
                    return Environment.TickCount + Game.Ping / 2 + 25 >= LastAaTick + Player.AttackDelay * 1000 &&
                           Attack;
                }
                return false;
            }
                    ...
                    if (ActiveMode == OrbwalkingMode.LaneClear || ActiveMode == OrbwalkingMode.Mixed)
                    {
                        foreach (var mob in
                            ObjectManager.Get<Obj_AI_Minion>()
                                .Where(
                                    mob =>
                                        mob.IsValidTarget() && (InAutoAttackRange(mob) || InSoldierAttackRange(mob)) &&
