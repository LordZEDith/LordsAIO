...
namespace LordsAIO.Champions
{
    ...
    {
        public static Obj_AI_Minion CloneR;
        public bool CloneRCreated;
        public bool CloneRFound;
        public Vector3 CloneRNearPosition;
        public int CloneRTick;
        public Obj_AI_Minion CloneW;
        public bool CloneWCreated;
        public bool CloneWFound;
        public int CloneWTick;
        public int Delay2 = 100;
        ...
        private void CastQEnemy()
        {
            ...
            if (Program.Menu.Item("useR_TeamFight").GetValue<bool>())
            {
                ...
                {
                    if (R.IsReady() && CloneR == null && Q.IsReady() && E.IsReady() &&
                        IsEnoughEnergy(
                            GetCost(SpellSlot.Q) + GetCost(SpellSlot.W) + GetCost(SpellSlot.E) + GetCost(SpellSlot.R)))
                    {
                        return;
                    }
                }
            ...
            if ((Program.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Mixed &&
                 Program.Menu.Item("useW_Harass").GetValue<bool>() ||
                 (Program.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo &&
                  Program.Menu.Item("useW_TeamFight").GetValue<bool>())))
            {
        ...
        private void CastWEnemy()
        {
            ...
            {
                if (ObjectManager.Player.Health * 100 / ObjectManager.Player.MaxHealth >
                    target.Health * 100 / target.MaxHealth &&
        ...
        private void CastE()
        {
            ...
            if (CloneW != null)
            {
                if (
                    Program.Helper.EnemyTeam.Any(
                        hero => (hero.Distance(CloneW.Position) < E.Range) && hero.IsValidTarget() && hero.IsVisible))
                {
                    E.Cast();
                    return;
                }
            ...
            if (CloneR != null)
            {
                if (
                    Program.Helper.EnemyTeam.Any(
                        hero => (hero.Distance(CloneR.Position) < E.Range) && hero.IsValidTarget() && hero.IsVisible))
                {
                    E.Cast();
                    return;
                }
            ...
            foreach (var minion in allMinions)
            {
                ...
                if ((ObjectManager.Player.GetSpellDamage(minion, SpellSlot.E) > minion.Health) ||
                    (ObjectManager.Player.GetSpellDamage(minion, SpellSlot.E) + 100 < minion.Health))
                {
                    E.Cast();
                }
            }
        ...
        private void CastR()
        {
            ...
            else if (ObjectManager.Player.Health * 100 / ObjectManager.Player.MaxHealth <
                     target.Health * 100 / target.MaxHealth)
            {
                if (CloneR.Position.Distance(target.Position) <
                    ObjectManager.Player.Position.Distance(target.Position))
                {
                    R.Cast();
                }
            }
        ...
        private void SearchForClone(string p)
        {
            ...
            if (p != null && p == "W")
            {
                shadow =
                    ObjectManager.Get<Obj_AI_Minion>()
                        .FirstOrDefault(hero => (hero.Name == "Shadow" && hero.IsAlly && (hero != CloneR)));
                if (shadow != null)
                {
                    CloneW = shadow;
                    CloneWFound = true;
            ...
            shadow =
                ObjectManager.Get<Obj_AI_Minion>()
                    .FirstOrDefault(
                        hero =>
                            ((hero.ServerPosition.Distance(CloneRNearPosition)) < 50) && hero.Name == "Shadow" &&
                            hero.IsAlly && hero != CloneW);
            if (shadow == null)
            {
                return;
            }
