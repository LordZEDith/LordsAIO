    ...
    internal class Teemo : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useW_TeamFight", "Use W if enemy flee").SetValue(true));
            ...
            Program.Menu.SubMenu("Drawing")
                .AddItem(new MenuItem("Draw_Vision", "Shroom Vision").SetValue(new Slider(1500, 2500, 1000)));
        ...
        private void CastW()
        {
            ...
            var target = TargetSelector.GetTarget(
                Orbwalking.GetRealAutoAttackRange(ObjectManager.Player) + 200, TargetSelector.DamageType.Physical);
        ...
        internal class ShroomTables
        {
            ...
            public ShroomTables()
            {
                CreateTables();
                var templist =
                    (from pos in HighPriority let x = pos.X let y = pos.Y let z = pos.Z select new Vector3(x, z, y))
                        .ToList();
                HighPriority = templist;
                templist =
                    (from pos in MediumPriority let x = pos.X let y = pos.Y let z = pos.Z select new Vector3(x, z, y))
                        .ToList();
                MediumPriority = templist;
                templist =
                    (from pos in LowPriority let x = pos.X let y = pos.Y let z = pos.Z select new Vector3(x, z, y))
                        .ToList();
