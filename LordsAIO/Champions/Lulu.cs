    ...
    internal class Lulu : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useW_TeamFight_assist", "Use W Assist").SetValue(true));
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useE_TeamFight_Aggro", "Use E pref Aggro").SetValue(false));
            Program.Menu.Item("useE_TeamFight_Aggro").ValueChanged += SwitchAggro;
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useE_TeamFight_Passiv", "Use E pref Passive").SetValue(true));
            ...
            Program.Menu.SubMenu("Harass")
                .AddItem(new MenuItem("useE_Harass_pix", "Use E on Minion for Q").SetValue(true));
        ...
        private void Interrupter_OnPosibleToInterrupt(Obj_AI_Hero target, Interrupter2.InterruptableTargetEventArgs args)
        {
            ...
            if (ObjectManager.Player.Distance(target) < W.Range && W.IsReady() && target.IsEnemy)
            {
                W.Cast(target);
            }
        }
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            switch (Program.Orbwalker.ActiveMode)
            {
                ...
                case Orbwalking.OrbwalkingMode.Mixed:
                    ...
                    if (Program.Menu.Item("useE_Harass_pix").GetValue<bool>() && EnoughManaFor(SpellSlot.Q, SpellSlot.E) &&
                        Q.IsReady())
                    {
        ...
        private void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                Drawing.DrawCircle(PixPosition(), 100, Color.Blue);
                if (Q.Level > 0)
                {
                    Drawing.DrawCircle(PixPosition(), QPix.Range, QPix.IsReady() ? Color.Green : Color.Red);
                }
            }
