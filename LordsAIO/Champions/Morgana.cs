    ...
    internal class Morgana : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useQ_TeamFight_Gapcloser", "Use Q Gapcloser").SetValue(true));
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useW_TeamFight_bind", "Use W if stunned").SetValue(true));
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useW_TeamFight_willhit", "Use W if hit").SetValue(new Slider(2, 5, 0)));
            Program.Menu.SubMenu("TeamFight")
        ...
        private void CastWEnemyAmount()
        {
            ...
            {
                W.CastIfWillHit(
                    enemy, Program.Menu.Item("useW_TeamFight_willhit").GetValue<Slider>().Value - 1, Packets());
                return;
            }
        }
        ...
        private void CastREnemyAmount()
        {
            ...
            if (ObjectManager.Player.CountEnemiesInRange((int) R.Range) >=
        ...
        private void CastWMinion()
        {
            ...
            var minions = MinionManager.GetMinions(
                ObjectManager.Player.Position, W.Range + (W.Width / 2), MinionTypes.All, MinionTeam.NotAlly);
            if (minions.Count == 0)
            {
                return;
            }
