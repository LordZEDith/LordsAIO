...
namespace LordsAIO.Champions
{
    internal class Riven : Champion
    {
        ...
        public static int StackPassive;
        ...
        private void LoadSpells()
        {
            ...
            E.SetSkillshot(
                0, Orbwalking.GetRealAutoAttackRange(Player), float.MaxValue, false, SkillshotType.SkillshotCircle);
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            StackPassive = firstOrDefault?.Count ?? 0;
            ...
            switch (Program.Orbwalker.ActiveMode)
            {
                case Orbwalking.OrbwalkingMode.Combo:
                    ...
                    if (Program.Menu.Item("useQ_TeamFight").GetValue<bool>() && (StackPassive <= 0 || QStage == 2))
                    {
                ...
                case Orbwalking.OrbwalkingMode.Mixed:
                    if (Program.Menu.Item("useQ_Harass").GetValue<bool>() && StackPassive == 0)
                    {
                        Cast_BasicCircleSkillshot_Enemy(QPred);
                    }
                ...
                case Orbwalking.OrbwalkingMode.LaneClear:
                    if (Program.Menu.Item("useQ_LaneClear").GetValue<bool>() && StackPassive == 0)
                    {
        ...
        private static void UseRSmart(Obj_AI_Base target)
        {
            ...
            else if (target is Obj_AI_Hero)
            {
                ...
                if (ObjectManager.Player.GetSpellDamage(target, SpellSlot.R) >
                    ((targ.Health + target.ScriptHealthBonus) - 5 * targ.Level) && R.IsReady())
                {
        ...
        public static void OnGameProcessPacket(GamePacketEventArgs args)
        {
            ...
            var gp = new GamePacket(args.PacketData) { Position = 5 };
        ...
        private void CastR()
        {
            ...
            else
            {
                ...
                if (ObjectManager.Player.GetSpellDamage(target, SpellSlot.R) >= target.Health ||
                    Environment.TickCount - RTick >= 13000 || !target.IsValidTarget(R.Range - 400))
                {
        ...
        private void CastE()
        {
            ...
            var target = TargetSelector.GetTarget(
                E.Range + Orbwalking.GetRealAutoAttackRange(Player), TargetSelector.DamageType.Physical);
            if (!target.IsValidTarget(E.Range + Orbwalking.GetRealAutoAttackRange(Player)) &&
                (target.Distance(ObjectManager.Player.ServerPosition) >= Orbwalking.GetRealAutoAttackRange(Player)))
            {
                return;
            }
