    ...
    internal class Gangplank : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            Program.Menu.SubMenu("TeamFight")
                .AddItem(new MenuItem("useR_TeamFight", "Use R").SetValue(new Slider(2, 0, 5)));
            ...
            Program.Menu.SubMenu("LaneClear")
                .AddItem(new MenuItem("useQ_LaneClear_minion", "Use Q Minion").SetValue(true));
            Program.Menu.SubMenu("LaneClear")
                .AddItem(new MenuItem("useQ_LaneClear_enemy", "Use Q Enemy").SetValue(true));
            ...
            Program.Menu.SubMenu("Passive")
                .AddItem(new MenuItem("useW_onLowlife", "Use W if Health below").SetValue(new Slider(40)));
        ...
        private void CastE()
        {
            ...
            if (Orbwalking.OrbwalkingMode.LaneClear == Program.Orbwalker.ActiveMode)
            {
                var allMinions = MinionManager.GetMinions(
                    ObjectManager.Player.Position, 400, MinionTypes.All, MinionTeam.NotAlly);
                ...
                string[] minionNames =
                {
                    "Worm", "Dragon", "LizardElder", "AncientGolem", "TT_Spiderboss", "TTNGolem",
                    "TTNWolf", "TTNWraith"
                };
                if (
                    !allMinions.Where(minion => minionNames.Contains(minion.Name))
                        .Any(
                            minion =>
                                ObjectManager.Get<Obj_AI_Hero>()
                                    .Count(hero => hero.IsAlly && minion.Distance(hero) <= 400) >= 3))
                {
                    return;
                }
                E.Cast();
            ...
            var count =
                ObjectManager.Get<Obj_AI_Hero>()
                    .Count(hero => hero.IsAlly && hero.Distance(ObjectManager.Player) <= E.Range);
            if (count >= 3)
            {
                E.Cast();
            }
