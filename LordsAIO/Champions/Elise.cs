...
namespace LordsAIO.Champions
{
    internal class Elise : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            Program.Menu.SubMenu("LaneClear")
                .AddItem(new MenuItem("LaneClear_mana", "Use Spider below Mana").SetValue(new Slider(40)));
        ...
        private void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Humanform())
            {
                if (Program.Menu.Item("Draw_Q_Human").GetValue<bool>())
                {
                    ...
                    {
                        Drawing.DrawCircle(
                            ObjectManager.Player.Position, QHuman.Range, QHuman.IsReady() ? Color.Green : Color.Red);
                    }
                }
                ...
                if (Program.Menu.Item("Draw_W_Human").GetValue<bool>())
                {
                    ...
                    {
                        Drawing.DrawCircle(
                            ObjectManager.Player.Position, WHuman.Range, WHuman.IsReady() ? Color.Green : Color.Red);
                    }
                }
                ...
                if (Program.Menu.Item("Draw_E_Human").GetValue<bool>())
                {
                    ...
                    {
                        Drawing.DrawCircle(
                            ObjectManager.Player.Position, EHuman.Range, EHuman.IsReady() ? Color.Green : Color.Red);
                    }
                }
            }
            else
            {
                if (Program.Menu.Item("Draw_Q_Spider").GetValue<bool>())
                {
                    ...
                    {
                        Drawing.DrawCircle(
                            ObjectManager.Player.Position, QSpider.Range, QSpider.IsReady() ? Color.Green : Color.Red);
                    }
                }
                ...
                if (Program.Menu.Item("Draw_E_Spider").GetValue<bool>())
                {
                    ...
                    {
                        Drawing.DrawCircle(
                            ObjectManager.Player.Position, ESpider.Range, ESpider.IsReady() ? Color.Green : Color.Red);
                    }
                }
            }
        }
                ...
                if (Program.Menu.Item("useW_LaneClear").GetValue<bool>())
                {
                    if (
                        MinionManager.GetMinions(
                            ObjectManager.Player.ServerPosition, Orbwalking.GetRealAutoAttackRange(ObjectManager.Player),
                            MinionTypes.All, MinionTeam.NotAlly, MinionOrderTypes.MaxHealth).Count >= 1)
                    {
                        WSpider.Cast();
                    }
                }
                if (
                    MinionManager.GetMinions(ObjectManager.Player.ServerPosition, 400, MinionTypes.All, MinionTeam.Ally)
                        .All(spider => spider.Name != "Spiderling") && !justSpider)
                {
                    Switchto("Human");
                }
            }
        ...
        private void CastWSpider()
        {
            var target = TargetSelector.GetTarget(
                Orbwalking.GetRealAutoAttackRange(ObjectManager.Player), TargetSelector.DamageType.Magical);
