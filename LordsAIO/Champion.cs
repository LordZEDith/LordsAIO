...
namespace LordsAIO
{
    internal class Champion
    {
        ...
        {
            Program.Menu.SubMenu(menuname)
                .AddItem(new MenuItem("ManaManager_" + menuname, "Mana-Manager").SetValue(new Slider(basicmana)));
        ...
        public Obj_AI_Hero Cast_BasicLineSkillshot_Enemy(Spell spell,
        ...
        public void Cast_BasicLineSkillshot_Enemy(Spell spell,
        ...
        public void Cast_BasicCircleSkillshot_Enemy(Spell spell,
            TargetSelector.DamageType damageType = TargetSelector.DamageType.Physical,
            float extrarange = 0)
        {
            if (!spell.IsReady() || !ManaManagerAllowCast(spell))
        ...
        public void Cast_BasicCircleSkillshot_AOE_Farm(Spell spell, int extrawidth = 0)
        {
            ...
            var minions = MinionManager.GetMinions(
                ObjectManager.Player.Position, spell.Range + ((spell.Width + extrawidth) / 2), MinionTypes.All,
                MinionTeam.NotAlly);
            if (minions.Count == 0)
            {
                return;
        ...
        public void Cast_Speedboost_onFriend(Spell spell)
        {
            ...
            var friend =
                Program.Helper.OwnTeam.FirstOrDefault(
                    x =>
                        x.Distance(ObjectManager.Player) <= spell.Range &&
                        Program.Helper.EnemyTeam.Any(
                            enemy =>
                                x.Distance(enemy) <= Orbwalking.GetRealAutoAttackRange(x) + 200 &&
                                x.BaseAttackDamage * x.AttackSpeedMod * 3 >= enemy.Health));
        ...
        public bool EnoughManaFor(SpellSlot spell,
            SpellSlot spell2 = SpellSlot.Unknown,
            SpellSlot spell3 = SpellSlot.Unknown,
            SpellSlot spell4 = SpellSlot.Unknown)
        {
